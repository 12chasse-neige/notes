...
#### 3. 设 $\sum_{n=1}^{\infty} a_{n}$ 是正项级数，满足 $\lim_{n \to \infty} a_{n} = 0$。证明：级数 $\sum_{n=1}^{\infty} \ln(1 + a_{n})$ 收敛当且仅当级数 $\sum_{n=1}^{\infty} a_{n}$ 收敛。

证明：

证法1: 显然级数 $\sum_{n=1}^{\infty} \ln(1 + a_{n})$ 为正项级数。

充分性：利用不等式 $\ln (1 + a_{n}) \leq a_{n}$ ，

因为级数 $\sum_{n=1}^{\infty} a_{n}$ 收敛，由比较定理，正项级数 $\sum_{n=1}^{\infty} \ln(1 + a_{n})$ 收敛。

必要性：
$$
\sum_{k=1}^{n} \ln(1 + a_{k}) = \ln \prod_{k=1}^{n}(1 + a_{k})
$$
