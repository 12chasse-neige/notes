...
设 $f, g : \mathbb{R}^{2} \rightarrow \mathbb{R}$ 的各个 2 阶偏导函数都存在且连续, $g(x_{0}, y_{0}) = 0$, $g_{y}(x_{0}, y_{0}) \neq 0$. 设方程 $g(x, y) = 0$ 在 $(x_{0}, y_{0})$ 附近确定 $C^{2}$ 光滑的隐函数 $y = y(x)$. 定义函数 $h : U \rightarrow \mathbb{R}$ 如下:
$$
h(x) = f(x, y(x)),
$$
...
(2) 假设对任何 $x \in U$, $f$ 的雅可比矩阵 $J_{f}(x)$ 都是可逆矩阵. 证明: $f^{-1} : V \rightarrow U$ 是 $C^{1}$ 光滑映射

证明

已经证明任何 $x \in U$, $f^{-1}$ 在 $f(x_{0})$ 处可微，又因为$J_{f}(x)$ 是可逆矩阵，因为$f : U \rightarrow V$ 是 $C^{1}$ 光滑的，所以$J_{f}$ 的矩阵元都是连续函数

利用链式法则
$$
J_{f^{-1}} (y) = (J_{f} (f^{-1} (y)))^{-1}
$$
由于$J_{f}(x)$ 是可逆的所以$J_{f^{-1}} (y)$ 的矩阵元均连续，所以对任何 $x \in U$, $f^{-1}$ 各个分量的偏导均是连续的，所以$f^{-1} : V \rightarrow U$ 是 $C^{1}$ 光滑映射。