...
### Understanding QFT
As a parametrization of the lack of constraining power of “L-inv. + locality + unitarity”.
### Constructing GR (classical tests)
...
$$
\Lambda \approx 10 \mathrm{TeV}
$$
$$
\mathcal{L} = \mathcal{L}_{free} + \mathcal{L}_{int} \quad \mathcal{L}_{int} \ll \mathcal{L}_{free}
$$
$$
\begin{gathered}
...
$$
$$
\mathcal{L}_{int} = \mathcal{L}_{strong} + \mathcal{L}_{EW} + \mathcal{L}_{gravity} + \mathcal{L}_{Higgs} + \mathcal{L}_{Yukawa}
$$
Move $\Lambda$ downward, at some scale, dramatic things happen.
...
proton $m_{p} \approx 938 \mathrm{MeV}$
...
### At $\Lambda \leq \mathrm{MeV}$: Yukawa int. Becomes IR Div.
Bound States: Nuclei (Nucleosynthesis)
Nuclei $e^{\pm}, \nu$, photon, graviton
When $E \leq \mathrm{MeV}, \quad E \leq m_{Nuclei}$, so we use Non-Rel. physics for nuclei
### At $\Lambda < 0 \mathrm{eV}$, QED Becomes IR div.
