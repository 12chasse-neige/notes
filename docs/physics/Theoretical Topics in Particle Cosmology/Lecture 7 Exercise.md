...
Plugging in those results, we have the difference of Lagrangian of spin-1 under LGT
...
$$
\Box_{x} e^{i p \cdot (x-y)} = -p^{2} e^{i p \cdot (x-y)}
$$

we have, up to the usual $i\epsilon$ prescription,

$$
\Box_{x} G^{\mu\nu,\rho\sigma}(x,y)
= \int \frac{\dd[4]{p}}{(2\pi)^{4}}e^{ip\cdot(x-y)}
\frac{i p^{2}}{p^{2}-i\epsilon}
\left[A(\eta^{\mu\rho}\eta^{\nu\sigma}+\eta^{\mu\sigma}\eta^{\nu\rho})+B\eta^{\mu\nu}\eta^{\rho\sigma}\right]
$$

Therefore, ignoring the contact effect of $\epsilon$ in the numerator,
...
Plugging this into (36), the tensor part must satisfy

$$
\frac{1}{4}
(\eta_{\lambda\mu}\eta_{\kappa\nu}+\eta_{\lambda\nu}\eta_{\kappa\mu}-\eta_{\lambda\kappa}\eta_{\mu\nu})
\left[A(\eta^{\mu\rho}\eta^{\nu\sigma}+\eta^{\mu\sigma}\eta^{\nu\rho})+B\eta^{\mu\nu}\eta^{\rho\sigma}\right]
= \frac{1}{2}(\delta^{\rho}_{\lambda}\delta^{\sigma}_{\kappa}+\delta^{\sigma}_{\lambda}\delta^{\rho}_{\kappa})
$$
