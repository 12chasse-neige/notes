...
The long-range effects come from the $q \to 0$ singularity, so we only consider the poles (which implies that the UV div is not important). We need one tech to analyse the singularity while avoid doing the loop integral.
...
2.  2 poles pinchs the contour from different directions on one point
